...
void CoreWorkload::Init(const utils::Properties &p) {
    ...
    if (request_dist == "uniform") {
        ...
        key_chooser_ =
            new SkewedLatestGenerator(*transaction_insert_key_sequence_);
...
ycsbc::Generator<uint64_t> *CoreWorkload::GetTxLenGenerator(
    ...
    int field_len =
        std::stoi(p.GetProperty(TX_LENGTH_PROPERTY, TX_LENGTH_DEFAULT));
    if (field_len_dist == "constant") {
        return new ConstGenerator(field_len);
    } else if (field_len_dist == "uniform") {
        return new UniformGenerator(1, field_len);
...
DB::Status CoreWorkload::TransactionBegin(DB &db) { return db.Begin(); }

DB::Status CoreWorkload::TransactionCommit(DB &db) { return db.Commit(); }

DB::Status CoreWorkload::TransactionAbort(DB &db) { return db.Abort(); }
