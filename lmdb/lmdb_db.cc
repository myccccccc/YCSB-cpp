...
void LmdbDB::Init() {
    ...
    if (ret) {
        throw utils::Exception(std::string("Init mdb_open: ") +
                               mdb_strerror(ret));
    }
...
void LmdbDB::DeserializeRowFilter(std::vector<Field> *values,
...
void LmdbDB::DeserializeRow(std::vector<Field> *values, const char *data_ptr,
                            size_t data_len) {
    const char *p = data_ptr;
    const char *lim = p + data_len;
    while (p != lim) {
...
DB::Status LmdbDB::ReadSingleEntry(const std::string &table,
    ...
    if (ret) {
        throw utils::Exception(std::string("Read mdb_get: ") +
                               mdb_strerror(ret));
    }
    if (fields != nullptr) {
        DeserializeRowFilter(&result, static_cast<char *>(val_slice.mv_data),
                             val_slice.mv_size, *fields);
    } else {
        DeserializeRow(&result, static_cast<char *>(val_slice.mv_data),
                       val_slice.mv_size);
    }
    mdb_txn_abort(txn);
    return kOK;
...
DB::Status LmdbDB::ScanSingleEntry(const std::string &table,
    ...
    for (int i = 0; !ret && i < len; i++) {
        ...
        if (fields != nullptr) {
            DeserializeRowFilter(&values,
                                 static_cast<char *>(val_slice.mv_data),
                                 val_slice.mv_size, *fields);
        } else {
            DeserializeRow(&values, static_cast<char *>(val_slice.mv_data),
                           val_slice.mv_size);
        }
...
DB::Status LmdbDB::UpdateSingleEntry(const std::string &table,
                                     const std::string &key,
    ...
    ret = mdb_get(txn, dbi_, &key_slice, &val_slice);
    if (ret) {
        throw utils::Exception(std::string("Update mdb_get: ") +
                               mdb_strerror(ret));
    }
    std::vector<Field> current_values;
    DeserializeRow(&current_values, static_cast<char *>(val_slice.mv_data),
                   val_slice.mv_size);
    ...
    if (ret) {
        throw utils::Exception(std::string("Update mdb_put: ") +
                               mdb_strerror(ret));
    }

    ret = mdb_txn_commit(txn);
    if (ret) {
        throw utils::Exception(std::string("Update mdb_txn_commit: ") +
                               mdb_strerror(ret));
    }
    return kOK;
}

DB::Status LmdbDB::InsertSingleEntry(const std::string &table,
                                     const std::string &key,
                                     std::vector<Field> &values) {
    MDB_txn *txn;
    MDB_val key_slice, val_slice;
    ...
    ret = mdb_put(txn, dbi_, &key_slice, &val_slice, 0);
    if (ret) {
        throw utils::Exception(std::string("Insert mdb_put: ") +
                               mdb_strerror(ret));
    }
    ret = mdb_txn_commit(txn);
    if (ret) {
        throw utils::Exception(std::string("Insert mdb_txn_commit: ") +
                               mdb_strerror(ret));
    }
    return kOK;
}

DB::Status LmdbDB::DeleteSingleEntry(const std::string &table,
                                     const std::string &key) {
    MDB_txn *txn;
    MDB_val key_slice;
    ...
    if (ret) {
        throw utils::Exception(std::string("Delete mdb_del: ") +
                               mdb_strerror(ret));
    }
    ret = mdb_txn_commit(txn);
    if (ret) {
        throw utils::Exception(std::string("Delete mdb_txn_commit: ") +
                               mdb_strerror(ret));
    }
    return kOK;
}
