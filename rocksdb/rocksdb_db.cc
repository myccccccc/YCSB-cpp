...
// merge operator disabled by default due to link error
    ...
    if (props.GetProperty(PROP_DESTROY, PROP_DESTROY_DEFAULT) == "true") {
        ...
        if (!s.ok()) {
            throw utils::Exception(std::string("RocksDB DestroyDB: ") +
                                   s.ToString());
        }
    }
    if (cf_descs.empty()) {
    ...
    if (options_file != "") {
        ...
        if (!s.ok()) {
            throw utils::Exception(
                std::string("RocksDB LoadOptionsFromFile: ") + s.ToString());
        }
    } else {
        ...
        val =
            std::stoi(props.GetProperty(PROP_MAX_BYTES_FOR_LEVEL_BASE,
        ...
        val = std::stoi(props.GetProperty(PROP_COMPACTION_PRI,
                                          PROP_COMPACTION_PRI_DEFAULT));
        if (val != -1) {
            opt->compaction_pri = static_cast<rocksdb::CompactionPri>(val);
        }
        val = std::stoi(props.GetProperty(PROP_MAX_OPEN_FILES,
        ...
        val = std::stoi(props.GetProperty(PROP_L0_STOP_TRIGGER,
        ...
        if (props.GetProperty(PROP_USE_DIRECT_READ,
                              PROP_USE_DIRECT_READ_DEFAULT) == "true") {
            opt->use_direct_reads = true;
        }
        if (props.GetProperty(PROP_USE_MMAP_WRITE,
                              PROP_USE_MMAP_WRITE_DEFAULT) == "true") {
        ...
        int bloom_bits = std::stoul(
            props.GetProperty(PROP_BLOOM_BITS, PROP_BLOOM_BITS_DEFAULT));
        if (bloom_bits > 0) {
